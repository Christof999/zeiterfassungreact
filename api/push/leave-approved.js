...
async function loadEmployeeSubscriptions(employeeId) {
  if (!employeeId) {
    return []
  }
...
function buildPayload(payload) {
  return JSON.stringify({
    ...
    body:
      payload.body ||
      `Ihr Antrag vom ${payload.periodLabel || ''} wurde genehmigt.`,
    url: '/vacation',
    icon: '/icon-192.png',
    badge: '/icon-192.png',
...
async function loadLeaveRequestForNotification(leaveRequestId) {
  ...
  return {
    ...
    alreadyNotified: !!leaveRequestData.approvalPushNotifiedAt
  }
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (leaveRequest.data.status !== 'approved') {
      return res.status(400).json({
        success: false,
        error: 'Urlaubsantrag ist nicht genehmigt.'
      })
    }
    ...
    if (leaveRequest.alreadyNotified) {
      return res.status(200).json({
        ...
        sent: 0,
        failed: 0,
        message: 'Push für diese Genehmigung wurde bereits versendet.'
      })
    }

    const employeeId = leaveRequest.data.employeeId || requestPayload.employeeId
    const subscriptions = await loadEmployeeSubscriptions(employeeId)

    if (subscriptions.length === 0) {
      await leaveRequest.ref.set(
        {
          approvalPushNotifiedAt: new Date(),
          approvalPushSent: 0,
          approvalPushFailed: 0,
          approvalPushMessage: 'Keine aktiven Push-Subscriptions für diesen Mitarbeiter',
          updatedAt: new Date()
        },
        { merge: true }
      )
    ...
    const payload = buildPayload({
      ...requestPayload,
      employeeId,
      leaveRequestId: requestPayload.leaveRequestId
    })
    let sent = 0
    let failed = 0
    ...
    await Promise.all(
      subscriptions.map(async (sub) => {
        try {
          ...
          console.error('Push Versand (Genehmigung) fehlgeschlagen:', {
            docId: sub.id,
            statusCode,
    ...
    await leaveRequest.ref.set(
      {
        approvalPushNotifiedAt: new Date(),
        approvalPushSent: sent,
        approvalPushFailed: failed,
        updatedAt: new Date()
      },
      { merge: true }
    )
    ...
    console.error('Push API Fehler (leave-approved):', error)
